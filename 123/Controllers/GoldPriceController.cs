...
public async Task<IActionResult> GetGoldPriceForChart()
{
    try
    {
        ...
        if (goldResponse.IsSuccessStatusCode && silverResponse.IsSuccessStatusCode)
        {
            ...
            // Kiểm tra nếu dữ liệu giá vàng và bạc hợp lệ
            if (goldPriceData == null || silverPriceData == null)
            {
                return StatusCode(500, "Dữ liệu không hợp lệ.");
            }
        ...
        {
            // Sử dụng goldResponse hoặc silverResponse để kiểm tra lỗi
            var errorResponse = goldResponse.IsSuccessStatusCode ? silverResponse : goldResponse;
            return StatusCode((int)errorResponse.StatusCode, "Không thể lấy dữ liệu từ API.");
        }
    }
    catch (HttpRequestException ex)
    {
